...
if __name__ == '__main__':
    ...
    pixels = image.reshape((-1, 3))  # 将图像数据展平为二维数组，每行是一个像素的RGB值
    ...
    # 重新将标签映射到图像并显示分割结果
    segmented_image = labels.reshape(image.shape[:-1])  # 恢复为与原图像相同的形状
    plt.imshow(segmented_image, cmap='jet')  # 使用jet色图进行可视化
    plt.show()